...
savedhiLogLevel savedhi_verbosity = savedhiLogLevelInfo;
FILE *savedhi_log_sink_file_target = NULL;

static savedhiLogSink **sinks;
static size_t sinks_count;

bool savedhi_log_sink_register(savedhiLogSink *sink) {

    if (!savedhi_realloc( &sinks, NULL, savedhiLogSink *, ++sinks_count )) {
        --sinks_count;
        return false;
    }
...
bool savedhi_log(savedhiLogLevel level, const char *file, int line, const char *function, const char *format, ...) {
    ...
    if (savedhi_verbosity < level)
        return false;

    va_list args;
    va_start( args, format );
    bool sunk = savedhi_vlog( level, file, line, function, format, &args );
    va_end( args );

    return sunk;
}

static const char *savedhi_log_formatter(savedhiLogEvent *event) {

    if (!event->formatted) {
        va_list args;
        va_copy( args, *(va_list *)event->args );
        event->formatted = savedhi_vstr( event->format, args );
        va_end( args );
    }
...
bool savedhi_vlog(savedhiLogLevel level, const char *file, int line, const char *function, const char *format, va_list *args) {
    ...
    savedhiLogEvent event = {
            ...
            .format = format,
            .args = args,
            .formatter = &savedhi_log_formatter,
    };
    bool sunk = savedhi_elog( &event );
...
bool savedhi_elog(savedhiLogEvent *event) {
    ...
    if (savedhi_verbosity < event->level)
        return false;

    bool sunk = false;
    if (!sinks_count)
        sunk = savedhi_log_sink_file( event );

    else
        for (unsigned int s = 0; s < sinks_count; ++s) {
            savedhiLogSink *sink = sinks[s];

            if (sink)
                sunk |= sink( event );
        }

    if (event->level <= savedhiLogLevelWarning) {
        (void)event->level/* error breakpoint opportunity */;
    }
    savedhi_free_string( &event->formatted );
    if (event->level <= savedhiLogLevelFatal)
        abort();

    return sunk;
}

bool savedhi_log_sink_file(savedhiLogEvent *event) {

    if (!savedhi_log_sink_file_target)
        savedhi_log_sink_file_target = stderr;

    if (savedhi_verbosity >= savedhiLogLevelDebug) {
        switch (event->level) {
            case savedhiLogLevelTrace:
                fprintf( savedhi_log_sink_file_target, "[TRC] " );
                break;
            case savedhiLogLevelDebug:
                fprintf( savedhi_log_sink_file_target, "[DBG] " );
                break;
            case savedhiLogLevelInfo:
                fprintf( savedhi_log_sink_file_target, "[INF] " );
                break;
            case savedhiLogLevelWarning:
                fprintf( savedhi_log_sink_file_target, "[WRN] " );
                break;
            case savedhiLogLevelError:
                fprintf( savedhi_log_sink_file_target, "[ERR] " );
                break;
            case savedhiLogLevelFatal:
                fprintf( savedhi_log_sink_file_target, "[FTL] " );
                break;
            default:
                fprintf( savedhi_log_sink_file_target, "[???] " );
                break;
        }
    }

    fprintf( savedhi_log_sink_file_target, "%s\n", event->formatter( event ) );
    return true;
}

void savedhi_uint16(const uint16_t number, uint8_t buf[static 2]) {

    buf[0] = (uint8_t)((number >> 8L) & UINT8_MAX);
    buf[1] = (uint8_t)((number >> 0L) & UINT8_MAX);
}

void savedhi_uint32(const uint32_t number, uint8_t buf[static 4]) {
...
void savedhi_uint64(const uint64_t number, uint8_t buf[static 8]) {
...
const char **savedhi_strings(size_t *count, const char *strings, ...) {
    ...
    *count = 0;
    ...
    for (const char *string = strings; string; (string = va_arg( args, const char * ))) {
        ...
        if (!savedhi_realloc( &array, &size, const char *, cursor + 1 )) {
            savedhi_free( &array, size );
            break;
        }
...
bool savedhi_buf_push_buf(uint8_t **buffer, size_t *bufferSize, const uint8_t *pushBuffer, const size_t pushSize) {
    ...
    if (!savedhi_realloc( buffer, bufferSize, uint8_t, (*bufferSize + pushSize) / sizeof( uint8_t ) )) {
        // realloc failed, we can't push.  Mark the buffer as broken.
        savedhi_free( buffer, *bufferSize );
...
bool savedhi_buf_push_uint32(uint8_t **buffer, size_t *bufferSize, const uint32_t pushInt) {

    uint8_t pushBuf[4 /* 32 / 8 */];
    savedhi_uint32( pushInt, pushBuf );
    return savedhi_buf_push( buffer, bufferSize, pushBuf, sizeof( pushBuf ) );
}

bool savedhi_buf_push_str(uint8_t **buffer, size_t *bufferSize, const char *pushString) {

    return pushString && savedhi_buf_push( buffer, bufferSize, (const uint8_t *)pushString, strlen( pushString ) );
}

bool savedhi_string_push(char **string, const char *pushString) {

    if (!string || !pushString)
        return false;

    // We overwrite an existing trailing NUL byte.
    return pushString && savedhi_buf_push( (uint8_t **const)string, &((size_t){ *string? strlen( *string ): 0 }),
            (const uint8_t *)pushString, strlen( pushString ) + 1 );
}

bool savedhi_string_pushf(char **string, const char *pushFormat, ...) {

    va_list args;
    va_start( args, pushFormat );
    const char *pushString = savedhi_vstr( pushFormat, args );
    bool success = savedhi_string_push( string, pushString );
    savedhi_free_string( &pushString );
    va_end( args );

    return success;
}

bool __savedhi_realloc(void **buffer, size_t *bufferSize, const size_t targetSize) {
...
void savedhi_zero(void *buffer, size_t bufferSize) {
...
bool __savedhi_free(void **buffer, const size_t bufferSize) {

    if (!buffer || !*buffer)
        return false;

    savedhi_zero( *buffer, bufferSize );
    free( *buffer );
    *buffer = NULL;
...
bool __savedhi_free_string(char **string) {

    return string && *string && __savedhi_free( (void **)string, strlen( *string ) );
}

bool __savedhi_free_strings(char **strings, ...) {

    bool success = true;

    va_list args;
    va_start( args, strings );
    success &= savedhi_free_string( strings );
    for (char **string; (string = va_arg( args, char ** ));)
        success &= savedhi_free_string( string );
    va_end( args );

    return success;
}

bool savedhi_kdf_scrypt(uint8_t *key, const size_t keySize, const uint8_t *secret, const size_t secretSize, const uint8_t *salt, const size_t saltSize,
...
#else
#error No crypto support for savedhi_kdf_scrypt.
#endif

    return true;
}

bool savedhi_kdf_blake2b(uint8_t *subkey, const size_t subkeySize, const uint8_t *key, const size_t keySize,
    ...
    if (crypto_generichash_blake2b_salt_personal( subkey, subkeySize, context, contextSize, key, keySize, saltBuf, personalBuf ) != OK)
        return false;
#else
#error No crypto support for savedhi_kdf_blake2b.
#endif

    return true;
}

bool savedhi_hash_hmac_sha256(uint8_t mac[static 32], const uint8_t *key, const size_t keySize, const uint8_t *message, const size_t messageSize) {
...
#else
#error No crypto support for savedhi_hash_hmac_sha256.
#endif
}

const static uint8_t *savedhi_aes(bool encrypt, const uint8_t *key, const size_t keySize, const uint8_t *buf, size_t *bufSize) {
...
//        return NULL;
    ...
    if (!aesBuf) {
//        savedhi_free( &resultBuf, aesSize );
        return NULL;
    }
...
const uint8_t *savedhi_aes_encrypt(const uint8_t *key, const size_t keySize, const uint8_t *plainBuffer, size_t *bufferSize) {

    return savedhi_aes( true, key, keySize, plainBuffer, bufferSize );
}

const uint8_t *savedhi_aes_decrypt(const uint8_t *key, const size_t keySize, const uint8_t *cipherBuffer, size_t *bufferSize) {

    return savedhi_aes( false, key, keySize, cipherBuffer, bufferSize );
}

#if UNUSED
const char *savedhi_hotp(const uint8_t *key, size_t keySize, uint64_t movingFactor, uint8_t digits, uint8_t truncationOffset) {

    // Hash the moving factor with the key.
    uint8_t counter[8];
    savedhi_uint64( movingFactor, counter );
    uint8_t hash[20];
    ...
    return savedhi_strdup( savedhi_str( "%0*d", digits, otp ) );
}
#endif
...
const char *savedhi_str(const char *format, ...) {
    ...
    va_list args;
    va_start( args, format );
    const char *str = savedhi_vstr( format, args );
    va_end( args );
...
const char *savedhi_vstr(const char *format, va_list args) {
    ...
    while (true) {
        ...
        if (chars < size)
            return str;
        if (!savedhi_realloc( &str, &size, char, chars + 1 ))
            break;
    }

    savedhi_free( &str, size );
    return NULL;
}
...
char *savedhi_hex(const uint8_t *buf, const size_t size, char *hex, size_t *hexSize) {
    ...
    if (!buf || !size)
        return NULL;
    if (!savedhi_realloc( &hex, hexSize, char, size * 2 + 1 ))
        return NULL;
...
const char *savedhi_hex_l(const uint32_t number, char hex[static 9]) {
    ...
    uint8_t buf[4 /* 32 / 8 */];
    savedhi_uint32( number, buf );

    size_t hexSize = 9;
    return savedhi_hex( buf, sizeof( buf ), hex, &hexSize );
}
...
size_t savedhi_base64_decode(const char *b64Text, uint8_t *byteBuf) {
    ...
    if (sodium_base642bin( byteBuf, savedhi_base64_decode_max( b64Length ), b64Text, b64Length,
...
size_t savedhi_base64_encode(const uint8_t *byteBuf, size_t byteSize, char *b64Text) {
    ...
    return strlen(
            sodium_bin2base64(
                    b64Text, savedhi_base64_encode_max( byteSize ),
...
size_t savedhi_utf8_char_count(const char *utf8String) {
    ...
    for (char *remaining = (char *)utf8String; remaining && *remaining; remaining += charlen, ++strchars)
        if (!(charlen = savedhi_utf8_char_size( remaining )))
            return 0;
...
const char *savedhi_strdup(const char *src) {
    ...
    return savedhi_memdup( src, len + 1 );
}
...
int savedhi_strcasecmp(const char *s1, const char *s2) {
    ...
    return savedhi_strncasecmp( s1, s2, s1 && s2? min( strlen( s1 ), strlen( s2 ) ): 0 );
}

int savedhi_strncasecmp(const char *s1, const char *s2, size_t max) {
